  ...
  submodules: [
    {
      ...
      lessons: [
        {
          ...
          steps: [
            ...
            },
            {
              ...
              html: ROOT,
              prompt: {
                en: 'Write `useList(initial)` returning `{ item, add, remove }` — `add(text)` appends, `remove(i)` removes by index — and a component that uses it with an input and a list.',
                ...
                en: [
                  {
                    ...
                    check:
                      'var result = null;\nfunction Probe() {\n  result = useList(["a"]);\n  return null;\n}\nvar container = document.createElement("div");\ndocument.body.appendChild(container);\nReactDOM.createRoot(container).render(React.createElement(Probe));\nawait tick(60);\nassert(result, "useList should return something");\nassert(Array.isArray(result.item), "item should be an array, got: " + typeof result.item);\nassert(result.item.join(",") === "a", "the initial value should be used, got: " + JSON.stringify(result.item));\nassert(typeof result.add === "function" && typeof result.remove === "function", "add and remove should be functions");\ncontainer.remove();',
                  },
                  {
                    name: { en: 'Each call keeps its own state', id: 'Tiap pemanggilan menyimpan state-nya sendiri' },
                    check:
                      'var a = null, b = null;\nfunction Probe() {\n  a = useList(["x"]);\n  b = useList([]);\n  return null;\n}\nvar container = document.createElement("div");\ndocument.body.appendChild(container);\nReactDOM.createRoot(container).render(React.createElement(Probe));\nawait tick(60);\nassert(a.item.length === 1 && b.item.length === 0, "the two calls should be independent");\ncontainer.remove();',
                  },
                  {
                    name: { en: 'The UI adds through the hook', id: 'Antarmukanya menambah lewat hook-nya' },
                    check:
                      'async function typeSubmit(text) {\n  var i = sel("#root input");\n  var setter = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(i), "value").set;\n  setter.call(i, text);\n  i.dispatchEvent(new Event("input", { bubbles: true }));\n  await tick(20);\n  var f = sel("#root form");\n  if (f) f.dispatchEvent(new Event("submit", { cancelable: true, bubbles: true }));\n  else await click("#root button");\n  await tick(40);\n}\nassert(sel("#root input"), "no input yet");\nvar before = all("#root li").length;\nawait typeSubmit("Learn");\nassert(all("#root li").length === before + 1, "one item should be added, got: " + all("#root li").length);',
                  },
                  {
                    name: { en: 'Removing works too', id: 'Penghapusan juga berfungsi' },
                    check:
                      'var result = null;\nfunction Probe() {\n  result = useList(["a", "b", "c"]);\n  return React.createElement("span", null, result.item.join(","));\n}\nvar container = document.createElement("div");\ndocument.body.appendChild(container);\nReactDOM.createRoot(container).render(React.createElement(Probe));\nawait tick(60);\nresult.remove(1);\nawait tick(60);\nassert(container.querySelector("span").textContent === "a,c", "remove(1) should drop the second item, got: " + JSON.stringify(container.querySelector("span").textContent));\ncontainer.remove();',
                  },
                ],
                id: [
                  {
              ...
              hints: [
                ...
                { en: 'add builds a new array with the spread, never push.', id: 'tambah membangun array baru dengan operator sebar, jangan push.' },
                { en: 'remove filters by index: item.filter((_, n) => n !== i)', id: 'hapus menyaring berdasarkan indeks: item.filter((_, n) => n !== i)' },
              ],
              solution: {
                en:
                  'function useList(initial = []) {\n  const [item, setItem] = React.useState(initial);\n\n  const add = (text) => setItem((d) => [...d, text]);\n  const remove = (i) => setItem((d) => d.filter((_, n) => n !== i));\n\n  return { item, add, remove };\n}\n\nfunction App() {\n  const { item, add, remove } = useList([]);\n  const [text, setText] = React.useState("");\n\n  function submit(e) {\n    e.preventDefault();\n    const trimmed = text.trim();\n    if (trimmed === "") return;\n    add(trimmed);\n    setText("");\n  }\n\n  return (\n    <div>\n      <form onSubmit={submit}>\n        <input value={text} onChange={(e) => setText(e.target.value)} />\n        <button type="submit">Add</button>\n      </form>\n      <ul>\n        {item.map((t, i) => (\n          <li key={i} onClick={() => remove(i)}>\n            {t}\n          </li>\n        ))}\n      </ul>\n    </div>\n  );\n}\n\nReactDOM.createRoot(document.querySelector("#root")).render(<App />);',
                id:
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              html: ROOT,
              prompt: {
                en: 'Split a small app: `Form({ onAdd })` owning its own text state, `List({ item })` rendering the list, and `App` owning the array and wiring them together.',
              ...
              starter: {
                en: 'function Form({ onAdd }) {\n\n}\n\nfunction List({ item }) {\n\n}\n\nfunction App() {\n\n}\n\nReactDOM.createRoot(document.querySelector("#root")).render(<App />);\n',
                id: 'function Form({ onTambah }) {\n\n}\n\nfunction Daftar({ item }) {\n\n}\n\nfunction Aplikasi() {\n\n}\n\nReactDOM.createRoot(document.querySelector("#root")).render(<Aplikasi />);\n',
              },
              tests: {
                en: [
                  {
                  ...
                  },
                  {
                    ...
                    check:
                      'async function submit(text) {\n  var i = sel("#root input");\n  var setter = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(i), "value").set;\n  setter.call(i, text);\n  i.dispatchEvent(new Event("input", { bubbles: true }));\n  await tick(20);\n  var f = sel("#root form");\n  if (f) f.dispatchEvent(new Event("submit", { cancelable: true, bubbles: true }));\n  else await click("#root button");\n  await tick(40);\n}\nvar before = all("#root li").length;\nawait submit("One");\nassert(all("#root li").length === before + 1, "an item should be added, got: " + all("#root li").length);\nassert(all("#root li")[all("#root li").length - 1].textContent.trim() === "One", "the last item\'s content is wrong");',
                  },
                  {
                    name: { en: 'List is presentational', id: 'Daftar hanya menampilkan' },
                    check:
                      'var container = document.createElement("div");\ndocument.body.appendChild(container);\nReactDOM.createRoot(container).render(React.createElement(List, { item: ["a", "b"] }));\nawait tick(60);\nassert(container.querySelectorAll("li").length === 2, "List should render one li per item, got: " + container.querySelectorAll("li").length);\nassert(container.querySelector("input") === null, "List should not contain an input — that is Form\'s job");\ncontainer.remove();',
                  },
                  {
                    name: { en: 'Form owns only its own text', id: 'Form hanya memiliki teksnya sendiri' },
                    check:
                      'var received = [];\nvar container = document.createElement("div");\ndocument.body.appendChild(container);\nReactDOM.createRoot(container).render(React.createElement(Form, { onAdd: function (t) { received.push(t); } }));\nawait tick(60);\nvar i = container.querySelector("input");\nassert(i, "Form should render an input");\nassert(container.querySelector("li") === null, "Form should not render a list");\nvar setter = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(i), "value").set;\nsetter.call(i, "From form");\ni.dispatchEvent(new Event("input", { bubbles: true }));\nawait tick(20);\nvar f = container.querySelector("form");\nif (f) f.dispatchEvent(new Event("submit", { cancelable: true, bubbles: true }));\nelse container.querySelector("button").click();\nawait tick(40);\nassert(received.length === 1 && received[0] === "From form", "Form should call onAdd with its text, received: " + JSON.stringify(received));\ncontainer.remove();',
                  },
                ],
                id: [
                  {
              ...
              hints: [
                { en: 'The array lives in App; the half-typed text lives in Form.', id: 'Array-nya tinggal di Aplikasi; teks setengah jadi tinggal di Form.' },
                { en: 'Form never sees the array, and List never sees the input.', id: 'Form tak pernah melihat array-nya, dan Daftar tak pernah melihat input-nya.' },
                { en: 'App passes onAdd down and receives the finished text back.', id: 'Aplikasi mengoper onTambah ke bawah dan menerima teks jadinya kembali.' },
              ],
              solution: {
                en:
                  'function Form({ onAdd }) {\n  const [text, setText] = React.useState("");\n\n  function submit(e) {\n    e.preventDefault();\n    const trimmed = text.trim();\n    if (trimmed === "") return;\n    onAdd(trimmed);\n    setText("");\n  }\n\n  return (\n    <form onSubmit={submit}>\n      <input value={text} onChange={(e) => setText(e.target.value)} />\n      <button type="submit">Add</button>\n    </form>\n  );\n}\n\nfunction List({ item }) {\n  return (\n    <ul>\n      {item.map((t, i) => (\n        <li key={i}>{t}</li>\n      ))}\n    </ul>\n  );\n}\n\nfunction App() {\n  const [item, setItem] = React.useState([]);\n\n  return (\n    <div>\n      <Form onAdd={(t) => setItem([...item, t])} />\n      <List item={item} />\n    </div>\n  );\n}\n\nReactDOM.createRoot(document.querySelector("#root")).render(<App />);',
                id:
      ...
      project: {
        ...
        requirements: [
          { en: '`useTasks()` returns `{ tasks, add, toggle, remove }` and owns the array.', id: '`useTugas()` mengembalikan `{ tugas, tambah, tandai, hapus }` dan memiliki array-nya.' },
          { en: '`Form({ onAdd })` owns its own text state and refuses empty entries.', id: '`Form({ onTambah })` memiliki state teksnya sendiri dan menolak masukan kosong.' },
          ...
          { en: 'Each `li` contains a `button.remove` that removes just that task.', id: 'Tiap `li` memuat `button.hapus` yang menghapus hanya tugas itu.' },
          { en: 'Three filter buttons — `All`, `Active`, `Done` — narrow what is listed.', id: 'Tiga tombol penyaring — `Semua`, `Aktif`, `Selesai` — mempersempit yang ditampilkan.' },
          { en: '`p#remaining` reads `2 of 3 not done`, counted from all tasks, not the filtered view.', id: '`p#sisa` bertuliskan `2 dari 3 belum selesai`, dihitung dari seluruh tugas, bukan dari tampilan tersaring.' },
        ],
        starter: {
          en: 'function useTasks() {\n\n}\n\nfunction Form({ onAdd }) {\n\n}\n\nfunction App() {\n\n}\n\nReactDOM.createRoot(document.querySelector("#root")).render(<App />);\n',
          id: 'function useTugas() {\n\n}\n\nfunction Form({ onTambah }) {\n\n}\n\nfunction Aplikasi() {\n\n}\n\nReactDOM.createRoot(document.querySelector("#root")).render(<Aplikasi />);\n',
        },
        tests: {
          en: [
            {
              ...
              check:
                'async function submit(text) {\n  var i = sel("#root input");\n  var setter = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(i), "value").set;\n  setter.call(i, text);\n  i.dispatchEvent(new Event("input", { bubbles: true }));\n  await tick(20);\n  var f = sel("#root form");\n  if (f) f.dispatchEvent(new Event("submit", { cancelable: true, bubbles: true }));\n  else await click("#root form button, #root button");\n  await tick(40);\n}\nassert(sel("#root input"), "no input yet");\nawait submit("One");\nawait submit("Two");\nawait submit("Three");\nassert(all("#root li").length === 3, "should be three tasks, got: " + all("#root li").length);\nassert(sel("#root input").value === "", "the input should be cleared");',
            },
            {
              name: { en: 'Empty entries are refused', id: 'Masukan kosong ditolak' },
              check:
                'async function submit(text) {\n  var i = sel("#root input");\n  var setter = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(i), "value").set;\n  setter.call(i, text);\n  i.dispatchEvent(new Event("input", { bubbles: true }));\n  await tick(20);\n  var f = sel("#root form");\n  if (f) f.dispatchEvent(new Event("submit", { cancelable: true, bubbles: true }));\n  await tick(40);\n}\nvar before = all("#root li").length;\nawait submit("");\nawait submit("   ");\nassert(all("#root li").length === before, "empty and whitespace should not add a task");',
            },
            {
              name: { en: 'Clicking toggles, and the count follows', id: 'Klik menandai, dan hitungannya mengikuti' },
              check:
                'function expected() {\n  var li = all("#root li");\n  var pending = li.filter(function (e) { return !e.classList.contains("done"); }).length;\n  return pending + " of " + li.length + " not done";\n}\nassert(sel("#remaining"), "no <p id=\\"remaining\\"> yet");\nassert(text("#remaining") === expected(), "#remaining should be " + JSON.stringify(expected()) + ", got: " + JSON.stringify(text("#remaining")));\nawait click(all("#root li")[0]);\nassert(all("#root li")[0].classList.contains("done"), "a click should mark it done");\nassert(text("#remaining") === expected(), "#remaining should update too, got: " + JSON.stringify(text("#remaining")));',
            },
            {
              name: { en: 'The filters narrow the list', id: 'Penyaringnya mempersempit daftar' },
              check:
                'function button(t) { return all("#root button").filter(function (b) { return b.textContent.trim() === t; })[0]; }\nassert(button("All") && button("Active") && button("Done"), "needs All, Active, and Done buttons");\nawait click(button("Active"));\nassert(all("#root li").length === 2, "Active should leave 2 tasks, got: " + all("#root li").length);\nall("#root li").forEach(function (e) { assert(!e.classList.contains("done"), "Active should not show done tasks"); });\nawait click(button("Done"));\nassert(all("#root li").length === 1, "Done should leave 1 task, got: " + all("#root li").length);\nawait click(button("All"));\nassert(all("#root li").length === 3, "All should show all three, got: " + all("#root li").length);',
            },
            {
              name: { en: 'The count ignores the filter', id: 'Hitungannya mengabaikan penyaring' },
              check:
                'function button(t) { return all("#root button").filter(function (b) { return b.textContent.trim() === t; })[0]; }\nawait click(button("Active"));\nassert(text("#remaining") === "2 of 3 not done", "#remaining should count all tasks even while filtered, got: " + JSON.stringify(text("#remaining")));\nawait click(button("All"));',
            },
            {
              name: { en: 'Delete removes just one', id: 'Hapus membuang satu saja' },
              check:
                'var before = all("#root li").length;\nvar target = all("#root li")[1].textContent;\nvar removeButton = all("#root li")[1].querySelector("button.remove");\nassert(removeButton, "each li needs a <button className=\\"remove\\">");\nawait click(removeButton);\nassert(all("#root li").length === before - 1, "should drop by one, got: " + all("#root li").length);\nvar remaining = all("#root li").map(function (e) { return e.textContent; });\nassert(remaining.indexOf(target) === -1, "the removed one should be the one clicked");',
            },
            {
              name: { en: 'The hook owns the data', id: 'Hook-nya memiliki datanya' },
              check:
                'var api = null;\nfunction Probe() {\n  api = useTasks();\n  return null;\n}\nvar container = document.createElement("div");\ndocument.body.appendChild(container);\nReactDOM.createRoot(container).render(React.createElement(Probe));\nawait tick(60);\nassert(api, "useTasks should return something");\nassert(Array.isArray(api.tasks), "tasks should be an array");\n["add", "toggle", "remove"].forEach(function (k) {\n  assert(typeof api[k] === "function", k + " should be a function");\n});\napi.add("Test");\nawait tick(60);\nassert(api.tasks.length >= 0, "add should not throw");\ncontainer.remove();\nassert(error() === null, "there is an error: " + error());',
            },
          ],
          id: [
            {
        ...
        hints: [
          ...
          { en: 'The filter is state in App, and the visible list is derived from it — never a second array in state.', id: 'Penyaringnya adalah state di Aplikasi, dan daftar yang tampak diturunkan darinya — jangan pernah jadi array kedua di state.' },
          ...
          { en: '#remaining counts from the full array, not from what the filter shows.', id: '#sisa menghitung dari array penuhnya, bukan dari yang ditampilkan penyaring.' },
        ],
        solution: {
          en:
            'function useTasks() {\n  const [tasks, setTasks] = React.useState([]);\n\n  const add = (text) => setTasks((d) => [...d, { id: Date.now() + Math.random(), text, done: false }]);\n  const toggle = (id) => setTasks((d) => d.map((t) => (t.id === id ? { ...t, done: !t.done } : t)));\n  const remove = (id) => setTasks((d) => d.filter((t) => t.id !== id));\n\n  return { tasks, add, toggle, remove };\n}\n\nfunction Form({ onAdd }) {\n  const [text, setText] = React.useState("");\n\n  function submit(e) {\n    e.preventDefault();\n    const trimmed = text.trim();\n    if (trimmed === "") return;\n    onAdd(trimmed);\n    setText("");\n  }\n\n  return (\n    <form onSubmit={submit}>\n      <input value={text} onChange={(e) => setText(e.target.value)} />\n      <button type="submit">Add</button>\n    </form>\n  );\n}\n\nfunction App() {\n  const { tasks, add, toggle, remove } = useTasks();\n  const [filter, setFilter] = React.useState("All");\n\n  const visible = tasks.filter((t) => {\n    if (filter === "Active") return !t.done;\n    if (filter === "Done") return t.done;\n    return true;\n  });\n\n  const pending = tasks.filter((t) => !t.done).length;\n\n  return (\n    <div>\n      <Form onAdd={add} />\n\n      <div>\n        {["All", "Active", "Done"].map((s) => (\n          <button key={s} onClick={() => setFilter(s)}>\n            {s}\n          </button>\n        ))}\n      </div>\n\n      <p id="remaining">\n        {pending} of {tasks.length} not done\n      </p>\n\n      <ul>\n        {visible.map((t) => (\n          <li key={t.id} className={t.done ? "done" : ""} onClick={() => toggle(t.id)}>\n            {t.text}\n            <button\n              className="remove"\n              onClick={(e) => {\n                e.stopPropagation();\n                remove(t.id);\n              }}\n            >\n              x\n            </button>\n          </li>\n        ))}\n      </ul>\n    </div>\n  );\n}\n\nReactDOM.createRoot(document.querySelector("#root")).render(<App />);',
          id:
