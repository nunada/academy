...
export type ModeId = 'python' | 'web' | 'javascript' | 'react' | 'sql' | 'typescript' | 'cpp' | 'game' | 'graph' | 'solid3d'
...
const GAME: Templat[] = [
  {
    ...
    // English-only by design, like every gamedev course file: `game.ts` calls
    // start/update/draw and switches on the "shape" dict keys by literal name,
    // so this is a real contract, not display text — see the gamedev course's
    // own rename commit for the full reasoning.
    code:
      'RADIUS = 8\n' +
      'WIDTH = 320\n' +
      'HEIGHT = 240\n\n' +
...
export const MODES: Mode[] = [
  ...
  { id: 'solid3d', label: { en: '3D Solid', id: 'Benda 3D' }, icon: '🧊', editorLabel: '', templat: [] },
]
