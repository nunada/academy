          ...
          steps: [
            {
              ...
              body: {
                en: 'Module 5 used "local maximum" and "local minimum" informally: a point where $f$ turns from increasing to decreasing, or back. Formally:\n\n| Kind | Condition | Compared against |\n|---|---|---|\n| **Local maximum** at $c$ | $f(c) \\geq f(x)$ | every $x$ near $c$ |\n| **Local minimum** at $c$ | $f(c) \\leq f(x)$ | every $x$ near $c$ |\n| **Absolute maximum** | $f(c) \\geq f(x)$ | **every** $x$ in the entire interval |\n\nA local claim says nothing about points far away; an absolute one is stronger. A function can have several local maxima with different heights — only the tallest one is also an absolute maximum, and on a closed interval the absolute extremum might not be a local one at all: it can sit right at an endpoint, where the function does not turn, it simply stops.',
                id: 'Modul 5 memakai "maksimum lokal" dan "minimum lokal" secara informal: titik tempat $f$ berbalik dari naik menjadi turun, atau sebaliknya. Secara formal:\n\n| Jenis | Syarat | Dibandingkan dengan |\n|---|---|---|\n| **Maksimum lokal** di $c$ | $f(c) \\geq f(x)$ | setiap $x$ di dekat $c$ |\n| **Minimum lokal** di $c$ | $f(c) \\leq f(x)$ | setiap $x$ di dekat $c$ |\n| **Maksimum mutlak** | $f(c) \\geq f(x)$ | **setiap** $x$ di seluruh interval |\n\nKlaim lokal tak mengatakan apa pun tentang titik yang jauh; klaim mutlak lebih kuat. Sebuah fungsi bisa punya beberapa maksimum lokal dengan tinggi berbeda-beda — hanya yang tertinggi yang juga maksimum mutlak, dan pada interval tertutup ekstrem mutlaknya mungkin bukan ekstrem lokal sama sekali: ia bisa duduk tepat di titik ujung, tempat fungsinya tak berbalik, ia sekadar berhenti.',
              },
              figure: {
                dim: 2,
                xSpan: [-2.5, 3.5],
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              body: {
                en: 'Two consequences follow:\n\n- If $f\'(x)=0$ for every $x$ on an interval, then $f$ is constant there. Proof:pick any two points $a<b$ in the interval; the Mean Value Theorem gives $f(b)-f(a) = f\'(c)(b-a) = 0\\cdot(b-a) = 0$, so $f(a)=f(b)$ — true for **any** two points, meaning $f$ never changes value.\n- If $f\'(x)=g\'(x)$ for every $x$ on an interval, then $f(x)=g(x)+C$ for some constant $C$. Proof: let $h(x)=f(x)-g(x)$; then $h\'(x)=f\'(x)-g\'(x)=0$ everywhere, so by the first consequence $h$ is constant. This is exactly the fact the Integral course built its entire Module 1 on — that any two antiderivatives of the same function differ only by a constant — stated there and finally proved here.',
                id: 'Dua akibat mengikuti:\n\n- Jika $f\'(x)=0$ untuk setiap $x$ pada suatu interval, maka $f$ konstan di situ. Bukti:ambil dua titik mana pun $a<b$ pada interval itu; Teorema Nilai Rata-rata memberi $f(b)-f(a) = f\'(c)(b-a) = 0\\cdot(b-a) = 0$, sehingga $f(a)=f(b)$ — benar untuk dua titik **mana pun**, berarti $f$ tak pernah berubah nilainya.\n- Jika $f\'(x)=g\'(x)$ untuk setiap $x$ pada suatu interval, maka $f(x)=g(x)+C$ untuk suatu konstanta $C$. Bukti: misalkan $h(x)=f(x)-g(x)$; maka $h\'(x)=f\'(x)-g\'(x)=0$ di mana-mana, sehingga menurut akibat pertama $h$ konstan. Inilah persis fakta yang menjadi dasar seluruh Modul 1 kursus Integral — bahwa dua antiturunan mana pun dari fungsi yang sama hanya berbeda oleh sebuah konstanta — dinyatakan di situ dan akhirnya dibuktikan di sini.',
              },
            },
            {
              kind: 'quiz',
