...
const SCHEMA = {
  en:
    ...
    `  id       INTEGER PRIMARY KEY,\n` +
    `  name     TEXT NOT NULL,\n` +
    `  class_id INTEGER REFERENCES class(id)   -- NULL = not yet placed\n` +
    `);\n\n` +
    `CREATE TABLE grade (\n` +
    `  id         INTEGER PRIMARY KEY,\n` +
  ...
  submodules: [
    {
          ...
          steps: [
            {
              ...
              body: {
                en: 'You could store the teacher\'s name on every student row. Then a teacher who changes school means editing thirty rows, and missing one leaves the database saying two different things at once. So the class lives in its own table, once, and each student keeps only a **foreign key**: `class_id`, the `id` of its class.',
              ...
              code: {
                en: 'class:    id=1, name=X-A, teacher=Bu Ratna\nstudent:  id=1, name=Adi, class_id=1   -- points to class 1',
            ...
            },
            {
              kind: 'concept',
              id: 'c3',
              title: { en: 'Aliases, and saying which name you mean', id: 'Alias, dan menyatakan nama yang mana' },
              body: {
                en: 'Both tables have a column called `name`, so a bare `name` is ambiguous. Write `student.name`, or give the table a short alias — `FROM student s` — and write `s.name`. Aliases are not decoration: past two tables, a query without them becomes very hard to read.',
              ...
              code: {
                en:
                  'SELECT s.name AS student, k.name AS class\n' +
                  'FROM student s\n' +
                  'JOIN class k ON k.id = s.class_id;',
                id:
            ...
            },
            {
              ...
              code: {
                en: 'SELECT s.name FROM student s JOIN class k ON k.id = s.class_id;',
                id: 'SELECT s.nama FROM siswa s JOIN kelas k ON k.id = s.kelas_id;',
              },
              options: [
            ...
            },
            {
              ...
              template: {
                en: 'SELECT s.name, k.teacher FROM student s ___ class k ___ k.id = s.class_id;',
                id: 'SELECT s.nama, k.guru FROM siswa s ___ kelas k ___ k.id = s.kelas_id;',
              },
              blanks: ['JOIN', 'ON'],
              explain: {
            ...
            },
            {
              ...
              schema: SCHEMA,
              prompt: {
                en: 'Return `student` (the student\'s name) and `class` (their class name) for every student who has a class, sorted by student name.',
                id: 'Kembalikan `siswa` (nama siswanya) dan `kelas` (nama kelasnya) untuk tiap siswa yang punya kelas, urut nama siswa.',
              },
              starter: { en: 'SELECT name AS student FROM student ORDER BY name;\n', id: 'SELECT nama AS siswa FROM siswa ORDER BY nama;\n' },
              tests: {
                en: [
                  {
                    name: { en: 'Two columns, student and class', id: 'Dua kolom, siswa dan kelas' },
                    expectColumns: ['student', 'class'],
                  },
                  {
                  ...
                  },
                  {
                    ...
                    setup: "UPDATE student SET class_id = 3 WHERE name = 'Bela';",
                    ordered: true,
                    expectRows: [
                      ['Adi', 'X-A'],
                      ['Bela', 'X-C'],
                  ...
                  },
                  {
                    ...
                    setup: "UPDATE student SET class_id = 2 WHERE name = 'Eka';",
                    ordered: true,
                    expectRows: [
                      ['Adi', 'X-A'],
                      ['Bela', 'X-A'],
              ...
              hints: [
                { en: 'Two tables in the FROM: student, joined to class.', id: 'Dua tabel di FROM: siswa, digabung ke kelas.' },
                { en: 'The rule is k.id = s.class_id — the key on one side, the id on the other.', id: 'Aturannya k.id = s.kelas_id — kuncinya di satu sisi, id-nya di sisi lain.' },
                { en: 'Both tables have `name`, so both need renaming with AS.', id: 'Kedua tabel punya `nama`, jadi keduanya perlu diganti nama dengan AS.' },
              ],
              solution: {
                en:
                  'SELECT s.name AS student, k.name AS class\n' +
          ...
          steps: [
            {
              ...
              code: {
                en:
                  'SELECT n.subject, n.score, s.name AS student, k.teacher\n' +
                  'FROM grade n\n' +
            ...
            },
            {
              ...
              code: {
                en:
                  'SELECT k.name AS class, COUNT(*) AS grade_count\n' +
                  'FROM grade n\n' +
                  'JOIN student s ON s.id = n.student_id\n' +
                  'JOIN class k ON k.id = s.class_id\n' +
                  'GROUP BY k.name;',
              ...
              output: { en: 'class  grade_count\nX-A    4\nX-B    4', id: 'kelas  banyak_nilai\nX-A    4\nX-B    4' },
            },
            {
              kind: 'quiz',
              id: 'q1',
            ...
            },
            {
              ...
              lines: {
                en: [
                  'SELECT n.subject, ROUND(AVG(n.score), 2) AS average',
                  'FROM grade n',
                  'JOIN student s ON s.id = n.student_id',
                  'GROUP BY n.subject',
            ...
            },
            {
              ...
              schema: SCHEMA,
              prompt: {
                en: 'For each class that has scores, return `class` and `average` — the average score, rounded to two decimals — best average first.',
              ...
              starter: { en: 'SELECT score FROM grade;\n', id: 'SELECT skor FROM nilai;\n' },
              tests: {
                en: [
                  {
                    name: { en: 'Two columns, class and average', id: 'Dua kolom, kelas dan rata_rata' },
                    expectColumns: ['class', 'average'],
                  },
                  {
                  ...
                  },
                  {
                    ...
                    setup: "INSERT INTO grade VALUES (9, 3, 'Chemistry', 100);",
                    ordered: true,
                    expectRows: [
                      ['X-B', 83.8],
                      ['X-A', 81.25],
                  ...
                  },
                  {
                    ...
                    setup: "UPDATE student SET class_id = 2 WHERE name = 'Bela';",
                    ordered: true,
                    expectRows: [
                      ['X-A', 81.5],
                      ['X-B', 80.17],
              ...
              hints: [
                { en: 'Start from `grade`, because that is where the scores live, then join your way to `class`.', id: 'Mulai dari `nilai`, karena di situlah skornya berada, lalu gabungkan sampai ke `kelas`.' },
                { en: 'Two joins: grade → student, then student → class.', id: 'Dua join: nilai → siswa, lalu siswa → kelas.' },
                { en: 'GROUP BY k.name, and ROUND(AVG(n.score), 2) for the average.', id: 'GROUP BY k.nama, dan ROUND(AVG(n.skor), 2) untuk rata-ratanya.' },
              ],
              solution: {
                en:
                  'SELECT k.name AS class, ROUND(AVG(n.score), 2) AS average\n' +
                  'FROM grade n\n' +
                  'JOIN student s ON s.id = n.student_id\n' +
                  'JOIN class k ON k.id = s.class_id\n' +
                  'GROUP BY k.name\n' +
      ...
      project: {
        ...
        requirements: [
          { en: 'Return five columns, in this order: `class`, `teacher`, `grade_count`, `average`, `highest`.', id: 'Kembalikan lima kolom, dalam urutan ini: `kelas`, `guru`, `banyak_nilai`, `rata_rata`, `tertinggi`.' },
          { en: 'Reach from `grade` through `student` to `class`.', id: 'Jangkau dari `nilai` lewat `siswa` sampai ke `kelas`.' },
          { en: '`average` is rounded to two decimals; `highest` is the best single score.', id: '`rata_rata` dibulatkan dua desimal; `tertinggi` adalah skor tunggal terbaik.' },
          { en: 'One row per class, best average first.', id: 'Satu baris per kelas, rata-rata terbaik dulu.' },
        ],
        starter: {
          en: 'SELECT n.subject, n.score\nFROM grade n\nJOIN student s ON s.id = n.student_id;\n',
          id: 'SELECT n.mapel, n.skor\nFROM nilai n\nJOIN siswa s ON s.id = n.siswa_id;\n',
        },
        tests: {
          en: [
            {
              name: { en: 'Five columns, in the right order', id: 'Lima kolom, dalam urutan yang benar' },
              expectColumns: ['class', 'teacher', 'grade_count', 'average', 'highest'],
            },
            {
            ...
            },
            {
              ...
              setup: "UPDATE class SET teacher = 'Pak Joko' WHERE name = 'X-A';",
              ordered: true,
              expectRows: [
            ...
            },
            {
              ...
              setup: "INSERT INTO grade VALUES (9, 1, 'Chemistry', 100);",
              ordered: true,
              expectRows: [
            ...
            },
            {
              ...
              setup: "INSERT INTO student VALUES (6, 'Fajar', 3);",
              ordered: true,
              expectRows: [
                ['X-A', 'Bu Ratna', 4, 81.25, 92],
                ['X-B', 'Pak Anwar', 4, 79.75, 95],
            ...
            },
            {
              ...
              setup: 'UPDATE grade SET score = 100 WHERE student_id = 3;',
              ordered: true,
              expectRows: [
        ...
        hints: [
          { en: 'Start the FROM at `grade` — every number you need lives there.', id: 'Mulai FROM-nya di `nilai` — semua angka yang kamu butuhkan ada di sana.' },
          { en: 'Group by both `k.name` and `k.teacher`: the teacher is not an aggregate, so it must be a grouping column.', id: 'Kelompokkan berdasarkan `k.nama` dan `k.guru`: gurunya bukan agregat, jadi ia harus jadi kolom pengelompokan.' },
          { en: 'MAX(n.score) for the best single score, ROUND(AVG(n.score), 2) for the average.', id: 'MAX(n.skor) untuk skor tunggal terbaik, ROUND(AVG(n.skor), 2) untuk rata-ratanya.' },
        ],
        solution: {
          en:
            'SELECT k.name AS class,\n' +
            '       k.teacher AS teacher,\n' +
            '       COUNT(*) AS grade_count,\n' +
            '       ROUND(AVG(n.score), 2) AS average,\n' +
            '       MAX(n.score) AS highest\n' +
            'FROM grade n\n' +
            'JOIN student s ON s.id = n.student_id\n' +
            'JOIN class k ON k.id = s.class_id\n' +
            'GROUP BY k.name, k.teacher\n' +
            'ORDER BY average DESC;',
          id:
    ...
    },
    {
          ...
          steps: [
            {
              ...
              code: {
                ...
                id: 'SELECT k.nama AS kelas, s.nama AS siswa\nFROM kelas k\nLEFT JOIN siswa s ON s.kelas_id = k.id;',
              },
              output: {
                en:
                  'class  student\n' +
            ...
            },
            {
              ...
              code: {
                en:
                  'SELECT k.name, COUNT(*) AS wrong, COUNT(s.id) AS correct\n' +
                  'FROM class k\n' +
                  'LEFT JOIN student s ON s.class_id = k.id\n' +
                  'GROUP BY k.name;',
                id:
            ...
            },
            {
              ...
              code: {
                en: 'SELECT s.name, k.name FROM student s LEFT JOIN class k ON k.id = s.class_id;',
                id: 'SELECT s.nama, k.nama FROM siswa s LEFT JOIN kelas k ON k.id = s.kelas_id;',
              },
              options: [
            ...
            },
            {
              ...
              schema: SCHEMA,
              prompt: {
                en: 'Return `class` and `student_count` for **every** class, including the one that has no students yet. Sort by class name.',
                id: 'Kembalikan `kelas` dan `banyak_siswa` untuk **setiap** kelas, termasuk yang belum punya siswa. Urut nama kelas.',
              },
              starter: {
                en:
                  'SELECT k.name AS class, COUNT(*) AS student_count\n' +
                  'FROM class k\n' +
                  'JOIN student s ON s.class_id = k.id\n' +
                  'GROUP BY k.name\n' +
                  'ORDER BY k.name;\n',
                id:
              ...
              tests: {
                en: [
                  {
                    name: { en: 'Two columns, class and student_count', id: 'Dua kolom, kelas dan banyak_siswa' },
                    expectColumns: ['class', 'student_count'],
                  },
                  {
                  ...
                  },
                  {
                    ...
                    setup: "UPDATE student SET class_id = 3 WHERE name IN ('Eka', 'Dina');",
                    ordered: true,
                    expectRows: [
                      ['X-A', 2],
                      ['X-B', 1],
                  ...
                  },
                  {
                    ...
                    setup: 'UPDATE student SET class_id = NULL WHERE class_id = 1;',
                    ordered: true,
                    expectRows: [
                      ['X-A', 0],
                      ['X-B', 2],
              ...
              hints: [
                ...
                { en: '`class` has to be the left table — it is the side you want kept whole.', id: '`kelas` harus jadi tabel kirinya — itu sisi yang ingin kamu jaga utuh.' },
              ...
              solution: {
                en:
                  'SELECT k.name AS class, COUNT(s.id) AS student_count\n' +
                  'FROM class k\n' +
                  'LEFT JOIN student s ON s.class_id = k.id\n' +
                  'GROUP BY k.name\n' +
                  'ORDER BY k.name;',
          ...
          steps: [
            {
              ...
              body: {
                en: 'A query in brackets can stand wherever a value can. `(SELECT AVG(score) FROM grade)` produces a single number, and the outer query compares against it. This is the answer to "above average" — a question you cannot ask with a plain `WHERE`, because the average is not known until every row has been read.',
              ...
              code: {
                en: 'SELECT subject, score\nFROM grade\nWHERE score > (SELECT AVG(score) FROM grade);',
                id: 'SELECT mapel, skor\nFROM nilai\nWHERE skor > (SELECT AVG(skor) FROM nilai);',
              },
            },
            {
              kind: 'concept',
              ...
              code: {
                en:
                  'SELECT name\nFROM student\n' +
                  "WHERE id IN (SELECT student_id FROM grade WHERE subject = 'Biology' AND score >= 90);",
                id:
                  'SELECT nama\nFROM siswa\n' +
            ...
            },
            {
              ...
              code: {
                en:
                  'SELECT s.name, AVG(n.score) AS average\n' +
                  'FROM student s\n' +
                  'JOIN grade n ON n.student_id = s.id\n' +
                  'GROUP BY s.name\n' +
                  'HAVING AVG(n.score) > (SELECT AVG(score) FROM grade);',
                id:
            ...
            },
            {
              ...
              code: {
                en: 'SELECT subject FROM grade WHERE score > (SELECT AVG(score) FROM grade);',
                id: 'SELECT mapel FROM nilai WHERE skor > (SELECT AVG(skor) FROM nilai);',
              },
              options: [
            ...
            },
            {
              ...
              schema: SCHEMA,
              prompt: {
                en: 'Return `student` and `average` (rounded to two decimals) for the students whose own average beats the average of every score in the school. Best first.',
              ...
              starter: {
                en:
                  'SELECT s.name AS student, ROUND(AVG(n.score), 2) AS average\n' +
                  'FROM student s\n' +
                  'JOIN grade n ON n.student_id = s.id\n' +
                  'GROUP BY s.name\n' +
                  'ORDER BY average DESC;\n',
              ...
              tests: {
                en: [
                  {
                    name: { en: 'Two columns, student and average', id: 'Dua kolom, siswa dan rata_rata' },
                    expectColumns: ['student', 'average'],
                  },
                  {
                  ...
                  },
                  {
                    ...
                    setup: "INSERT INTO grade VALUES (9, 3, 'Chemistry', 10), (10, 3, 'Physics', 10);",
                    ordered: true,
                    expectRows: [
                      ['Dina', 87],
                      ['Adi', 81.5],
                  ...
                  },
                  {
                    ...
                    setup: "INSERT INTO grade VALUES (9, 3, 'Chemistry', 100), (10, 3, 'Physics', 100), (11, 3, 'History', 100);",
                    ordered: true,
                    expectRows: [
                      ['Candra', 89],
                      ['Dina', 87],
              ...
              hints: [
                ...
                { en: 'HAVING AVG(n.score) > (SELECT AVG(score) FROM grade)', id: 'HAVING AVG(n.skor) > (SELECT AVG(skor) FROM nilai)' },
              ],
              solution: {
                en:
                  'SELECT s.name AS student, ROUND(AVG(n.score), 2) AS average\n' +
      ...
      project: {
        ...
        requirements: [
          { en: 'Return five columns, in this order: `student`, `class`, `grade_count`, `average`, `gap`.', id: 'Kembalikan lima kolom, dalam urutan ini: `siswa`, `kelas`, `banyak_nilai`, `rata_rata`, `selisih`.' },
          { en: 'Every student appears, even with no class and no scores.', id: 'Tiap siswa muncul, bahkan yang tanpa kelas dan tanpa nilai.' },
          { en: '`class` is NULL for an unplaced student; `grade_count` is 0 when there are no scores.', id: '`kelas` bernilai NULL untuk siswa yang belum ditempatkan; `banyak_nilai` bernilai 0 saat tak ada nilai.' },
          { en: '`average` is their average rounded to two decimals, and `gap` is that average minus the school average, also rounded to two decimals.', id: '`rata_rata` adalah rata-ratanya dibulatkan dua desimal, dan `selisih` adalah rata-rata itu dikurangi rata-rata sekolah, juga dibulatkan dua desimal.' },
        ...
        tests: {
          en: [
            {
              name: { en: 'Five columns, in the right order', id: 'Lima kolom, dalam urutan yang benar' },
              expectColumns: ['student', 'class', 'grade_count', 'average', 'gap'],
            },
            {
            ...
            },
            {
              ...
              setup: "UPDATE student SET class_id = 3 WHERE name = 'Eka';",
              ordered: true,
              expectRows: [
                ['Adi', 'X-A', 2, 81.5, 1],
                ['Bela', 'X-A', 2, 81, 0.5],
            ...
            },
            {
              ...
              setup: "INSERT INTO student VALUES (6, 'Fajar', NULL);",
              ordered: true,
              expectRows: [
                ['Adi', 'X-A', 2, 81.5, 1],
                ['Bela', 'X-A', 2, 81, 0.5],
            ...
            },
            {
              ...
              setup: "INSERT INTO grade VALUES (9, 5, 'Chemistry', 100), (10, 5, 'Physics', 100);",
              ordered: true,
              expectRows: [
                ['Adi', 'X-A', 2, 81.5, -2.9],
                ['Bela', 'X-A', 2, 81, -3.4],
        ...
        hints: [
          { en: 'Start the FROM at `student`: it is the side that must survive, so it goes on the left of both joins.', id: 'Mulai FROM-nya di `siswa`: itu sisi yang harus selamat, jadi ia ada di kiri kedua join-nya.' },
          { en: 'Two LEFT JOINs — one to class, one to grade.', id: 'Dua LEFT JOIN — satu ke kelas, satu ke nilai.' },
          ...
          { en: 'The school average is a subquery: (SELECT AVG(score) FROM grade). Subtract it, then round the whole thing.', id: 'Rata-rata sekolah adalah subkueri: (SELECT AVG(skor) FROM nilai). Kurangkan, lalu bulatkan keseluruhannya.' },
        ],
        solution: {
          en:
            ...
            '       ROUND(AVG(n.score), 2) AS average,\n' +
            '       ROUND(AVG(n.score) - (SELECT AVG(score) FROM grade), 2) AS gap\n' +
            'FROM student s\n' +
            'LEFT JOIN class k ON k.id = s.class_id\n' +
            'LEFT JOIN grade n ON n.student_id = s.id\n' +
