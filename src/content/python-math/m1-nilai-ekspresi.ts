          ...
          steps: [
            {
              ...
              figure: {
                ...
                ticks: true,
                items: [
                  { t: 'point', at: [1, 1], label: 'A', color: 'a' },
                  { t: 'point', at: [4, 5], label: 'B', color: 'b' },
                  { t: 'seg', from: [1, 1], to: [4, 5], color: 'result', label: 'd' },
                  { t: 'seg', from: [1, 1], to: [4, 1], color: 'muted', dashed: true, label: 'dx' },
                  { t: 'seg', from: [4, 1], to: [4, 5], color: 'muted', dashed: true, label: 'dy' },
                  { t: 'right', at: [4, 1], from: [1, 1], to: [4, 5] },
                ],
                caption: {
                  en: 'sqrt is exactly the tool that turns a horizontal leg (dx = 3) and a vertical leg (dy = 4) into the straight-line distance d = sqrt(dx² + dy²) = 5 — the idea behind the project just ahead.',
                  id: 'sqrt persis alat yang mengubah sisi mendatar (dx = 3) dan sisi tegak (dy = 4) menjadi jarak garis lurus d = sqrt(dx² + dy²) = 5 — ide di balik proyek yang akan datang.',
                },
              },
              code: 'import math\nprint(math.sqrt(16))\nprint(math.sqrt(2))',
              output: '4.0\n1.4142135623730951',
