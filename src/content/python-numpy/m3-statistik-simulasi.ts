          ...
          steps: [
            ...
            },
            {
              ...
              figure: {
                ...
                height: 140,
                items: [
                  { t: 'dot', x: 70, y: 0, color: 'a', label: '70' },
                  { t: 'dot', x: 80, y: 0, color: 'a', label: '80' },
                  { t: 'dot', x: 90, y: 0, color: 'a', label: '90' },
                  { t: 'dot', x: 100, y: 0, color: 'a', label: '100' },
                  { t: 'vline', x: 85, color: 'result', dashed: true, label: 'mean = 85' },
                ],
                caption: {
                  en: 'The four values plotted along a line, and their mean marked as the dashed balance point — np.std measures how far, on average, the dots sit from that line.',
                  id: 'Keempat nilainya digambar sepanjang garis, dan rata-ratanya ditandai sebagai titik keseimbangan putus-putus — np.std mengukur seberapa jauh, rata-rata, titik-titiknya dari garis itu.',
                },
              },
          ...
          steps: [
            {
              ...
              figure: {
                ...
                ticks: true,
                items: [
                  { t: 'curve', f: 'sqrt(1-x^2)', from: 0, to: 1, color: 'a' },
                  { t: 'dot', x: 0.1, y: 0.2, color: 'result' },
                  { t: 'dot', x: 0.3, y: 0.4, color: 'result' },
                  { t: 'dot', x: 0.2, y: 0.6, color: 'result' },
                  { t: 'dot', x: 0.5, y: 0.3, color: 'result' },
                  { t: 'dot', x: 0.6, y: 0.5, color: 'result' },
                  { t: 'dot', x: 0.4, y: 0.7, color: 'result' },
                  { t: 'dot', x: 0.15, y: 0.85, color: 'result' },
                  { t: 'dot', x: 0.55, y: 0.6, color: 'result' },
                  { t: 'dot', x: 0.8, y: 0.7, color: 'muted' },
                  { t: 'dot', x: 0.9, y: 0.5, color: 'muted' },
                  { t: 'dot', x: 0.7, y: 0.8, color: 'muted' },
                  { t: 'dot', x: 0.95, y: 0.4, color: 'muted' },
                ],
                caption: {
                  en: 'A point lands inside the quarter circle exactly when x² + y² ≤ 1 — 8 of these 12 do, and 4 * 8 / 12 ≈ 2.67 is already in the right neighborhood of π.',
                  id: 'Sebuah titik jatuh di dalam seperempat lingkaran persis ketika x² + y² ≤ 1 — 8 dari 12 titik ini begitu, dan 4 * 8 / 12 ≈ 2,67 sudah berada di lingkungan yang tepat untuk π.',
                },
              },
            },
            {
