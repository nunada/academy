  ...
  submodules: [
    /* ------------------------------------------------------------ 3.1 tables */
    {
      ...
      lessons: [
        {
          ...
          steps: [
            ...
            },
            {
              ...
              body: {
                en: 'A header cell is `<th>`, not a bold `<td>`. It is what a screen reader repeats before each value, so a listener hears "Score: 80" instead of a bare number.',
            ...
            },
            {
              ...
              code: {
                en: '<table>\n  <caption>Exam scores, class 10A</caption>\n  <tr><th>Name</th><th>Score</th></tr>\n  <tr><td>Ani</td><td>80</td></tr>\n</table>',
            ...
            },
            {
              ...
              id: 'w1',
              prompt: {
                en: 'Build a table with a header row `Item` / `Price`, then two data rows: `Pencil` `3000` and `Book` `15000`.',
              ...
              tests: {
                en: [
                  {
                    name: { en: 'Three rows', id: 'Three rows' },
                    check: 'assert(all("table tr").length === 3, "there must be three <tr>, found: " + all("table tr").length);',
                  },
                  {
                    name: { en: 'The first row uses header cells', id: 'The first row uses header cells' },
                    check:
                      'var th = all("table tr")[0].querySelectorAll("th");\nassert(th.length === 2, "the first row must have two <th>");\nassert(th[0].textContent.trim() === "Item", "first header: Item");\nassert(th[1].textContent.trim() === "Price", "second header: Price");',
                  },
                  {
                    name: { en: 'The data rows are right', id: 'The data rows are right' },
                    check:
                      'var r = all("table tr");\nvar want = [["Pencil", "3000"], ["Book", "15000"]];\nfor (var i = 0; i < 2; i++) {\n  var td = r[i + 1].querySelectorAll("td");\n  assert(td.length === 2, "a data row must have two <td>");\n  assert(td[0].textContent.trim() === want[i][0], "cell must be: " + want[i][0]);\n  assert(td[1].textContent.trim() === want[i][1], "cell must be: " + want[i][1]);\n}',
                  },
                ],
                id: [
                  {
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              id: 'w1',
              prompt: {
                en: 'Build a table with the caption `Score List`, a `thead` with `Name` / `Score`, a `tbody` with one row `Ani` `80`, and a final row in the tbody spanning both columns with the text `Total: 80`.',
              ...
              tests: {
                en: [
                  {
                    name: { en: 'The caption comes first', id: 'The caption comes first' },
                    check:
                      'var cap = sel("table > caption");\nassert(cap, "there is no <caption> inside <table> yet");\nassert(cap.textContent.trim() === "Score List", "caption must be: Score List");\nassert(sel("table").firstElementChild === cap, "the caption must be the first child of <table>");',
                  },
                  {
                    name: { en: 'thead holds the header row', id: 'thead holds the header row' },
                    check:
                      'var th = all("table thead th");\nassert(th.length === 2, "thead must have two <th>");\nassert(th[0].textContent.trim() === "Name");\nassert(th[1].textContent.trim() === "Score");',
                  },
                  {
                    name: { en: 'tbody holds the data row', id: 'tbody holds the data row' },
                    check:
                      'var r = all("table tbody tr");\nassert(r.length === 2, "tbody must have two rows, found: " + r.length);\nvar td = r[0].querySelectorAll("td");\nassert(td[0].textContent.trim() === "Ani" && td[1].textContent.trim() === "80", "the data row must be Ani / 80");',
                  },
                  {
                    name: { en: 'The last row spans both columns', id: 'The last row spans both columns' },
                    check:
                      'var last = all("table tbody tr")[1].querySelectorAll("td");\nassert(last.length === 1, "the last row needs just one cell, found: " + last.length);\nassert(last[0].getAttribute("colspan") === "2", "that cell needs colspan=\\"2\\"");\nassert(last[0].textContent.trim() === "Total: 80", "its content must be: Total: 80");',
                  },
                ],
                id: [
                  {
      ...
      project: {
        ...
        requirements: [
          { en: 'Full document with the title `Schedule`.', id: 'Dokumen lengkap dengan judul `Jadwal`.' },
          { en: 'A table with the caption `Class Schedule`.', id: 'Sebuah tabel dengan caption `Jadwal Pelajaran`.' },
          { en: 'A `thead` with three headers: `Time`, `Monday`, `Tuesday`.', id: 'Sebuah `thead` dengan tiga header: `Jam`, `Senin`, `Selasa`.' },
        ...
        tests: {
          en: [
            {
              name: { en: 'Caption and column headers', id: 'Caption and column headers' },
              check:
                'assert(text("table > caption") === "Class Schedule", "caption must be: Class Schedule");\nvar th = all("table thead th");\nassert(th.length === 3, "thead must have three <th>, found: " + th.length);\nvar want = ["Time", "Monday", "Tuesday"];\nfor (var i = 0; i < 3; i++) assert(th[i].textContent.trim() === want[i], "header " + (i + 1) + " must be: " + want[i]);',
            },
            {
              name: { en: 'Three body rows', id: 'Three body rows' },
              check: 'assert(all("table tbody tr").length === 3, "tbody must have three rows, found: " + all("table tbody tr").length);',
            },
            {
              name: { en: 'Each row has three cells', id: 'Each row has three cells' },
              check:
        ...
        hints: [
          ...
          { en: 'A body row looks like: <tr><th>07:00</th><td>Math</td><td>Language</td></tr>', id: 'Baris badannya berbentuk: <tr><th>07.00</th><td>Matematika</td><td>Bahasa</td></tr>' },
          { en: 'Order inside the table: caption, thead, tbody.', id: 'Urutan di dalam tabel: caption, thead, tbody.' },
        ],
        solution: {
          en: '<!doctype html>\n<html lang="en">\n  <head>\n    <meta charset="utf-8">\n    <title>Schedule</title>\n  </head>\n  <body>\n    <table>\n      <caption>Class Schedule</caption>\n      <thead>\n        <tr><th>Time</th><th>Monday</th><th>Tuesday</th></tr>\n      </thead>\n      <tbody>\n        <tr><th>07:00</th><td>Math</td><td>Language</td></tr>\n        <tr><th>09:00</th><td>Physics</td><td>History</td></tr>\n        <tr><th>11:00</th><td>Computer Science</td><td>PE</td></tr>\n      </tbody>\n    </table>\n  </body>\n</html>',
    ...
    /* ------------------------------------------------------------- 3.2 forms */
    {
          ...
          steps: [
            {
              ...
              code: {
                en: '<input type="text" placeholder="Name">\n<input type="email" placeholder="Email">\n<input type="number" placeholder="Age">',
                id: '<input type="text" placeholder="Nama">\n<input type="email" placeholder="Email">\n<input type="number" placeholder="Umur">',
              },
              preview: true,
            },
            {
              ...
              code: {
                en: '<label for="name">Full name</label>\n<input type="text" id="name" name="name">',
                id: '<label for="nama">Nama lengkap</label>\n<input type="text" id="nama" name="nama">',
              },
              preview: true,
            },
            {
            ...
            },
            {
              ...
              id: 'q1',
              prompt: {
                en: 'What connects `<label for="age">` to its field?',
                id: 'Apa yang menghubungkan `<label for="umur">` dengan isiannya?',
              },
              options: [
                { en: 'The input\'s id="age"', id: 'Atribut id="umur" pada input' },
                { en: 'The input\'s name="age"', id: 'Atribut name="umur" pada input' },
            ...
            },
            {
              ...
              template: {
                en: '<label ___="city">City</label>\n<input type="text" ___="city" name="city">',
                id: '<label ___="kota">Kota</label>\n<input type="text" ___="kota" name="kota">',
              },
              blanks: ['for', 'id'],
              explain: {
            ...
            },
            {
              ...
              tests: {
                en: [
                  {
                    name: { en: 'Both fields exist with the right types', id: 'Both fields exist with the right types' },
                    check:
                  ...
                  {
                    name: { en: 'Both send a value, and email is required', id: 'Both send a value, and email is required' },
                    check:
                      'assert(attr("#name", "name") === "name", "the name input needs name=name");\nassert(attr("#email", "name") === "email", "the email input needs name=email");\nassert(sel("#email").hasAttribute("required"), "email must be required");',
                  },
                ],
                id: [
                  {
          ...
          steps: [
            {
              ...
              code: {
                en: '<label for="grade">Grade</label>\n<select id="grade" name="grade">\n  <option value="10">Grade 10</option>\n  <option value="11">Grade 11</option>\n</select>',
                id: '<label for="kelas">Kelas</label>\n<select id="kelas" name="kelas">\n  <option value="10">Kelas 10</option>\n  <option value="11">Kelas 11</option>\n</select>',
              },
              preview: true,
            },
            {
              ...
              code: {
                en: '<input type="radio" id="m" name="gender" value="M">\n<label for="m">Male</label>\n<input type="radio" id="f" name="gender" value="F">\n<label for="f">Female</label>',
                id: '<input type="radio" id="l" name="jk" value="L">\n<label for="l">Laki-laki</label>\n<input type="radio" id="p" name="jk" value="P">\n<label for="p">Perempuan</label>',
              },
              preview: true,
            },
            {
              ...
              body: {
                en: '`<textarea>` is for long text and, unlike `<input>`, has a closing tag. `<button type="submit">` sends the form. Say what it does — "Send message" beats "Submit".',
            ...
            },
            {
              ...
              tests: {
                en: [
                  ...
                  {
                    name: { en: 'Two options with the right values', id: 'Two options with the right values' },
                    check:
                      'var o = all("select#city option");\nassert(o.length === 2, "there must be two <option>, found: " + o.length);\nassert(o[0].getAttribute("value") === "sby" && o[0].textContent.trim() === "Surabaya", "first option: Surabaya / sby");\nassert(o[1].getAttribute("value") === "mlg" && o[1].textContent.trim() === "Malang", "second option: Malang / mlg");',
                  },
                  {
                    name: { en: 'A submit button', id: 'A submit button' },
                    check:
                      'var b = sel("form button");\nassert(b, "there is no <button> yet");\nassert(b.getAttribute("type") === "submit", "the button must be type=submit");\nassert(b.textContent.trim() === "Send", "the button must say: Send");',
                  },
                ],
                id: [
                  {
      ...
      project: {
        ...
        requirements: [
          { en: 'Full document with the title `Registration`.', id: 'Dokumen lengkap dengan judul `Pendaftaran`.' },
          { en: 'A `form` containing: text `name`, email `email`, number `age`.', id: 'Sebuah `form` berisi: teks `nama`, email `email`, angka `umur`.' },
          { en: 'A `select` named `course` with at least two options.', id: 'Sebuah `select` bernama `kursus` dengan minimal dua opsi.' },
          { en: 'A `textarea` named `reason`.', id: 'Sebuah `textarea` bernama `alasan`.' },
          ...
          { en: 'A submit button. `name` and `email` are required.', id: 'Sebuah tombol kirim. `nama` dan `email` wajib diisi.' },
        ],
        starter: {
        ...
        tests: {
          en: [
            {
              name: { en: 'The three inputs, with the right types', id: 'The three inputs, with the right types' },
              check:
                'var want = { name: "text", email: "email", age: "number" };\nfor (var k in want) {\n  var el = sel("form input#" + k);\n  assert(el, "there is no input with id " + k + " yet");\n  assert(el.getAttribute("type") === want[k], k + " must be type=" + want[k]);\n  assert(el.getAttribute("name") === k, k + " needs name=" + k);\n}',
            },
            {
              name: { en: 'The select offers real choices', id: 'The select offers real choices' },
              check:
                'var s = sel("form select");\nassert(s, "there is no <select> yet");\nassert(s.getAttribute("name") === "course", "select needs name=course");\nassert(s.querySelectorAll("option").length >= 2, "select needs at least two option");',
            },
            {
              name: { en: 'The textarea is there', id: 'The textarea is there' },
              check:
            ...
            {
              name: { en: 'Required fields and a submit button', id: 'Required fields and a submit button' },
              check:
                'assert(sel("#name").hasAttribute("required"), "name must be required");\nassert(sel("#email").hasAttribute("required"), "email must be required");\nvar b = sel("form button, form input[type=submit]");\nassert(b, "there is no submit button yet");',
            },
          ],
          id: [
            {
