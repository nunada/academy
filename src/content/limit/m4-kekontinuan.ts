          ...
          steps: [
            {
              ...
              figure: {
                ...
                ySpan: [-1, 5],
                ticks: true,
                params: [{ name: 'd', min: -1.5, max: -0.02, step: 0.02, value: -0.8, label: 'x−2' }],
                items: [
                  { t: 'curve', f: 'x+1', from: -1, to: 2, color: 'a' },
                  { t: 'dot', x: 2, y: 3, open: true, color: 'a' },
                  { t: 'dot', x: 2, y: 1, color: 'result' },
                  { t: 'dot', x: '2+d', y: '(2+d)+1', color: 'c', label: 'x' },
                ],
                caption: {
                  en: 'Drag $x-2$ toward $0$ from the left — the moving point slides along the curve straight into the hollow spot at height $3$. But $f(2) = 1$ is declared elsewhere, the filled dot off the curve — a removable discontinuity. Re-defining $f(2)$ to be $3$ would patch it completely, which is exactly what makes this kind different from a jump.',
                  id: 'Geser $x-2$ menuju $0$ dari kiri — titik yang bergerak meluncur sepanjang kurva tepat menuju titik kosong pada tinggi $3$. Tetapi $f(2) = 1$ dideklarasikan di tempat lain, titik penuh yang jauh dari kurva — diskontinuitas yang bisa dihapus. Mendefinisikan ulang $f(2)$ menjadi $3$ akan menambalnya sepenuhnya, dan itulah yang membuat jenis ini berbeda dari lompatan.',
                },
              },
            },
            {
          ...
          steps: [
            {
              ...
              figure: {
                ...
                ySpan: [-4, 6],
                ticks: true,
                params: [{ name: 'c', min: 0, max: 4, step: 0.1, value: 2, label: 'c' }],
                items: [
                  { t: 'curve', f: 'x^2-3*x-1', color: 'a' },
                  { t: 'dot', x: 0, y: -1, color: 'b' },
                  { t: 'dot', x: 4, y: 3, color: 'b' },
                  { t: 'hline', y: 0, color: 'muted', dashed: true },
                  { t: 'dot', x: 'c', y: 'c^2-3*c-1', color: 'result', label: 'f(c)' },
                ],
                caption: {
                  en: 'Slide $c$ across $[0,4]$ and watch $f(c)$ sweep continuously from $-1$ up to $3$ — since $f(0)$ and $f(4)$ have opposite signs, it must cross the dashed $x$-axis somewhere along the way, a root the IVT guarantees without computing it.',
                  id: 'Geser $c$ di sepanjang $[0,4]$ dan amati $f(c)$ menyapu secara kontinu dari $-1$ hingga $3$ — karena $f(0)$ dan $f(4)$ berlawanan tanda, ia pasti memotong sumbu $x$ putus-putus di suatu tempat di jalan, sebuah akar yang dijamin TNA tanpa menghitungnya.',
                },
              },
            },
            {
