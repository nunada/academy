          ...
          steps: [
            {
              ...
              body: {
                en: 'For any real number $n$,\n$$\\frac{d}{dx}\\big(x^n\\big) = nx^{n-1}$$\nModule 1 proved the case $n=2$ by hand: $\\frac{d}{dx}(x^2) = 2x$, exactly this formula. The **power rule** says the same expanding-and-cancelling argument works for every power, so there is never a need to redo it.\n\nTwo companions come for free:\n\n- The **constant rule**, $\\frac{d}{dx}(c) = 0$ — a constant function is a flat line, slope zero everywhere, which is also $n=0$ inside the power rule\'s own logic in disguise.\n- The **constant multiple rule**, $\\frac{d}{dx}\\big(c\\,f(x)\\big) = c\\,f\'(x)$ — scaling a function scales its tangent slopes by the same factor.',
                id: 'Untuk sebarang bilangan real $n$,\n$$\\frac{d}{dx}\\big(x^n\\big) = nx^{n-1}$$\nModul 1 membuktikan kasus $n=2$ secara manual: $\\frac{d}{dx}(x^2) = 2x$, persis rumus ini. **Aturan pangkat** mengatakan argumen jabarkan-lalu-coret yang sama berlaku untuk setiap pangkat, jadi tak pernah perlu mengulanginya.\n\nDua pendamping datang secara gratis:\n\n- **Aturan konstanta**, $\\frac{d}{dx}(c) = 0$ — fungsi konstan adalah garis datar, kemiringannya nol di mana-mana, yang juga $n=0$ dalam logika aturan pangkat itu sendiri yang menyamar.\n- **Aturan kelipatan konstanta**, $\\frac{d}{dx}\\big(c\\,f(x)\\big) = c\\,f\'(x)$ — menskalakan fungsi menskalakan kemiringan garis singgungnya dengan faktor yang sama.',
              },
            },
            {
              kind: 'concept',
              ...
              figure: {
                ...
                items: [
                  ...
                  { t: 'dot', x: 'x0', y: 'x0^3', color: 'b', label: 'P' },
                  { t: 'dot', x: 'x0', y: '3*x0^2', color: 'result', label: "f'(x₀)" },
                ],
                caption: {
                  en: 'Move $x_0$: the tangent slope at $P$ on the parabola-like curve $x^3$ always matches the height read off $f\'(x)=3x^2$ directly below — never negative, matching that $x^3$ never actually decreases, only flattens exactly at $x=0$.',
                  id: 'Geser $x_0$: kemiringan garis singgung di $P$ pada kurva $x^3$ selalu cocok dengan tinggi yang dibaca dari $f\'(x)=3x^2$ tepat di bawahnya — tak pernah negatif, cocok dengan kenyataan $x^3$ tak pernah benar-benar turun, hanya sempat mendatar tepat di $x=0$.',
                },
              },
            },
            {
            ...
            },
            {
              ...
              figure: {
                ...
                items: [
                  ...
                  { t: 'curve', f: '(x0^3-3*x0^2)+(3*x0^2-6*x0)*(x-x0)', color: 'b', dashed: true, label: 'tangent' },
                  { t: 'dot', x: 'x0', y: 'x0^3-3*x0^2', color: 'b', label: 'P' },
                  { t: 'dot', x: 'x0', y: '3*x0^2-6*x0', color: 'result', label: "f'(x₀)" },
                ],
                caption: {
                  en: 'Move $x_0$ and watch the tangent at $P$ flatten and reverse exactly where the $f\'$ curve dips below the axis (between $x=0$ and $x=2$) — that dip is $f$ itself sloping downward.',
                  id: 'Geser $x_0$ dan lihat garis singgung di $P$ mendatar lalu berbalik arah persis tempat kurva $f\'$ turun di bawah sumbu (antara $x=0$ dan $x=2$) — dan itulah $f$ sendiri melandai turun.',
                },
              },
            },
            {
