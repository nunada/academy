            ...
            },
            {
              ...
              figure: {
                ...
                items: [
                  ...
                  { t: 'dot', x: 'x0', y: '(2*x0-1)^3', color: 'b', label: 'A' },
                ],
                caption: {
                  en: 'Move $x_0$: for $y=(2x-1)^3$, $y\' = 6(2x-1)^2$, so at $x_0=1$ the tangent has slope $6(1)^2=6$ — the chain rule\'s answer, drawn.',
                  id: 'Geser $x_0$: untuk $y=(2x-1)^3$, $y\' = 6(2x-1)^2$, sehingga di $x_0=1$ garis singgungnya berkemiringan $6(1)^2=6$ — jawaban aturan rantai, digambarkan.',
                },
              },
            },
            {
            ...
            },
            {
              ...
              figure: {
                ...
                items: [
                  ...
                  { t: 'dot', x: 'x0', y: 'sqrt(5*x0+1)', color: 'b', label: 'A' },
                ],
                caption: {
                  en: 'Move $x_0$: as the outer layer flattens out (a root grows more and more slowly), the tangent at $A$ visibly loses its steepness — matching how $y\'=\\frac{5}{2\\sqrt{5x+1}}$ shrinks as $x$ grows.',
                  id: 'Geser $x_0$: saat lapisan luarnya mendatar (akar tumbuh makin lambat), garis singgung di $A$ tampak kehilangan kecuraman — cocok dengan $y\'=\\frac{5}{2\\sqrt{5x+1}}$ yang mengecil saat $x$ membesar.',
                },
              },
            },
            {
          ...
          steps: [
            {
              ...
              figure: {
                ...
                ySpan: [-6, 8],
                ticks: true,
                params: [{ name: 'theta', min: 0.15, max: 1.42, step: 0.05, value: 0.93, label: 'θ' }],
                items: [
                  { t: 'curve', f: 'sqrt(25-x^2)', from: -5, to: 5, color: 'a' },
                  { t: 'curve', f: '-sqrt(25-x^2)', from: -5, to: 5, color: 'a' },
                  { t: 'dot', x: '5*cos(theta)', y: '5*sin(theta)', color: 'result', label: 'A' },
                  { t: 'curve', f: '(5-cos(theta)*x)/sin(theta)', color: 'b', dashed: true, label: 'tangent' },
                ],
                caption: {
                  en: 'Drag $\\theta$ to slide $A$ around the circle: the tangent line always has slope $-x/y$, found from $x^2+y^2=25$ without ever solving for $y$ as a formula in $x$.',
                  id: 'Geser $\\theta$ untuk menggerakkan $A$ mengelilingi lingkaran: garis singgungnya selalu berkemiringan $-x/y$, ditemukan dari $x^2+y^2=25$ tanpa pernah menyelesaikan $y$ sebagai rumus dalam $x$.',
                },
              },
            },
            {
          ...
          steps: [
            {
              ...
              body: {
                en: 'A term like $xy$ mixes the two variables, and differentiating it needs the **product rule**, with $x$ as one factor and $y$ — standing in for a function of $x$ — as the other:\n$$\\frac{d}{dx}(xy) = (1)(y) + (x)\\frac{dy}{dx} = y + x\\frac{dy}{dx}$$\nThe recipe for any implicit curve is now complete. Differentiate every term, using:\n\n- the power rule plainly on $x$-only terms,\n- the chain rule on every $y$,\n- the product rule the moment $x$ and $y$ appear multiplied together in the same term.\n\nThen collect every $\\frac{dy}{dx}$ on one side and solve.',
                id: 'Suku seperti $xy$ mencampur kedua variabelnya, dan menurunkannya memerlukan **aturan hasil kali**, dengan $x$ sebagai satu faktor dan $y$ — mewakili fungsi dari $x$ — sebagai faktor yang lain:\n$$\\frac{d}{dx}(xy) = (1)(y) + (x)\\frac{dy}{dx} = y + x\\frac{dy}{dx}$$\nResep untuk kurva implisit mana pun kini lengkap. Turunkan tiap suku, dengan memakai:\n\n- aturan pangkat biasa pada suku yang hanya $x$,\n- aturan rantai pada setiap $y$,\n- aturan hasil kali begitu $x$ dan $y$ muncul dikalikan bersama dalam suku yang sama.\n\nLalu kumpulkan setiap $\\frac{dy}{dx}$ ke satu ruas dan selesaikan.',
              },
            },
            {
              kind: 'concept',
              ...
              figure: {
                ...
                items: [
                  ...
                  { t: 'curve', f: '6/x0-(6/x0^2)*(x-x0)', color: 'b', dashed: true, label: 'tangent' },
                ],
                caption: {
                  en: 'Move $x_0$ along $xy=6$: the tangent\'s slope is always $-y/x$, steep near the axis and gentle further out — read straight off the picture as $A$ slides.',
                  id: 'Geser $x_0$ sepanjang $xy=6$: kemiringan garis singgungnya selalu $-y/x$, curam dekat sumbu dan landai jauh darinya — terbaca langsung dari gambar saat $A$ bergeser.',
                },
              },
            },
            {
