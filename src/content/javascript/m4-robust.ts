  ...
  submodules: [
    {
          ...
          steps: [
            {
              ...
              output: { en: 'failed to read data\nprogram continues', id: 'gagal membaca data\nprogram lanjut' },
            },
            {
              kind: 'concept',
              id: 'c2',
            ...
            },
            {
              ...
              template: {
                en: 'function register(name) {\n  if (name === "") {\n    ___ new Error("name is required");\n  }\n  return name;\n}',
                id: 'function daftar(nama) {\n  if (nama === "") {\n    ___ new Error("nama wajib diisi");\n  }\n  return nama;\n}',
              },
              blanks: ['throw'],
              explain: {
            ...
            },
            {
              ...
              js: true,
              prompt: {
                en: 'Write `divide(a, b)` returning `a / b`, but throwing `Error("divide by zero")` when `b` is 0. Then write `safeDivide(a, b)` that calls it and returns the string `"cannot"` instead of throwing.',
              ...
              starter: { en: 'function divide(a, b) {\n\n}\n\nfunction safeDivide(a, b) {\n\n}\n', id: 'function bagi(a, b) {\n\n}\n\nfunction amanBagi(a, b) {\n\n}\n' },
              tests: {
                en: [
                  {
                    name: { en: 'divide divides', id: 'divide divides' },
                    check: 'assert(divide(10, 4) === 2.5, "divide(10, 4) must be 2.5, currently: " + JSON.stringify(divide(10, 4)));',
                  },
                  {
                    name: { en: 'divide throws with the right message', id: 'divide throws with the right message' },
                    check:
                      'var threw = false;\ntry { divide(5, 0); } catch (e) { threw = true; assert(e.message === "divide by zero", "the message must be \\"divide by zero\\", currently: " + JSON.stringify(e.message)); }\nassert(threw, "divide(5, 0) should throw an Error");',
                  },
                  {
                    name: { en: 'safeDivide catches instead', id: 'safeDivide catches instead' },
                    check:
                      'assert(safeDivide(10, 4) === 2.5, "safeDivide(10, 4) must be 2.5");\nassert(safeDivide(5, 0) === "cannot", "safeDivide(5, 0) must be \\"cannot\\", currently: " + JSON.stringify(safeDivide(5, 0)));',
                  },
                  {
                    name: { en: 'safeDivide never throws', id: 'safeDivide never throws' },
                    check:
                      'try { safeDivide(1, 0); } catch (e) { assert(false, "safeDivide must not throw — it must catch it"); }',
                  },
                ],
                id: [
                  {
                    name: { en: 'divide divides', id: 'bagi membagi' },
                    check: 'assert(bagi(10, 4) === 2.5, "bagi(10, 4) harus 2.5, sekarang: " + JSON.stringify(bagi(10, 4)));',
                  },
                  {
                    name: { en: 'divide throws with the right message', id: 'bagi melempar dengan pesan yang benar' },
                    check:
                  ...
                  {
                    name: { en: 'safeDivide catches instead', id: 'amanBagi menangkapnya' },
                    check:
                  ...
                  {
                    name: { en: 'safeDivide never throws', id: 'amanBagi tidak pernah melempar' },
                    check:
              ...
              hints: [
                ...
                { en: 'safeDivide wraps the call in try/catch.', id: 'amanBagi membungkus pemanggilannya dalam try/catch.' },
                { en: 'return divide(a, b) inside try; return "cannot" inside catch.', id: 'return bagi(a, b) di dalam try; return "tidak bisa" di dalam catch.' },
              ],
              solution: {
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              body: {
                en: 'Reading `student.address.city` when there is no `address` throws. Optional chaining `?.` gives `undefined` at the first missing link instead — useful exactly where data arrives from somewhere you do not control.',
            ...
            },
            {
              ...
              js: true,
              prompt: {
                en: 'Write `readProfile(text)` that parses the JSON and returns `{ name, city }`. Use `"No name"` when `name` is missing, `"?"` when `address.city` is missing, and return `null` when the text is not valid JSON.',
              ...
              starter: { en: 'function readProfile(text) {\n\n}\n', id: 'function bacaProfil(teks) {\n\n}\n' },
              tests: {
                en: [
                  {
                    name: { en: 'A complete profile', id: 'A complete profile' },
                    check:
                      'var h = readProfile(\'{"name":"Ani","address":{"city":"Surabaya"}}\');\nassert(h.name === "Ani" && h.city === "Surabaya", "the result: " + JSON.stringify(h));',
                  },
                  {
                    name: { en: 'Missing pieces get their fallbacks', id: 'Missing pieces get their fallbacks' },
                    check:
                      'var h = readProfile("{}");\nassert(h.name === "No name", "name must be \\"No name\\", currently: " + JSON.stringify(h.name));\nassert(h.city === "?", "city must be \\"?\\", currently: " + JSON.stringify(h.city));',
                  },
                  {
                    name: { en: 'An empty name is kept, not replaced', id: 'An empty name is kept, not replaced' },
                    check:
                      'var h = readProfile(\'{"name":""}\');\nassert(h.name === "", "an empty name is valid — use ?? not ||, currently: " + JSON.stringify(h.name));',
                  },
                  {
                    name: { en: 'Broken JSON gives null', id: 'Broken JSON gives null' },
                    check:
                      'assert(readProfile("not json") === null, "broken text must give null, currently: " + JSON.stringify(readProfile("not json")));\nassert(error() === null, "do not let an error escape: " + error());',
                  },
                ],
                id: [
                  {
              ...
              hints: [
                ...
                { en: 'data.address?.city ?? "?"', id: 'data.alamat?.kota ?? "?"' },
              ],
              solution: {
                en: 'function readProfile(text) {\n  let data;\n  try {\n    data = JSON.parse(text);\n  } catch (err) {\n    return null;\n  }\n\n  return {\n    name: data.name ?? "No name",\n    city: data.address?.city ?? "?",\n  };\n}',
      ...
      project: {
        ...
        requirements: [
          { en: 'Keep the records in an array named `students`.', id: 'Simpan catatannya dalam array bernama `kelas`.' },
          { en: 'Submitting adds `{ name, score }` and clears both fields.', id: 'Submit menambahkan `{ nama, nilai }` dan mengosongkan kedua isian.' },
          { en: 'An empty name, or a score that is not a number from 0 to 100, is refused: `#error` reads `Invalid input` and nothing is added.', id: 'Nama kosong, atau nilai yang bukan angka 0 sampai 100, ditolak: `#galat` bertuliskan `Masukan tidak valid` dan tidak ada yang ditambahkan.' },
          { en: 'A valid entry clears `#error` and appends an `li` reading `Ani: 88`.', id: 'Masukan yang sah mengosongkan `#galat` dan menambahkan `li` bertuliskan `Ani: 88`.' },
          { en: '`#summary` reads `2 students, average 76.5`, or `No data yet` when empty.', id: '`#ringkas` bertuliskan `2 siswa, rata-rata 76.5`, atau `Belum ada data` bila kosong.' },
          { en: 'Expose `save()` returning the array as JSON text, and `load(text)` replacing it from JSON — returning false and changing nothing when the text is broken.', id: 'Sediakan `simpan()` yang mengembalikan array-nya sebagai teks JSON, dan `muat(teks)` yang menggantinya dari JSON — mengembalikan false dan tidak mengubah apa pun bila teksnya rusak.' },
        ],
        starter: { en: 'const students = [];\n', id: 'const kelas = [];\n' },
        tests: {
          en: [
            {
              name: { en: 'A valid entry is added and rendered', id: 'A valid entry is added and rendered' },
              check:
                'function submit(n, v) { sel("#name").value = n; sel("#score").value = v; sel("#form").dispatchEvent(new Event("submit", { cancelable: true, bubbles: true })); }\nsubmit("Ani", "88");\nassert(students.length === 1, "students must contain one record, currently: " + students.length);\nassert(students[0].name === "Ani" && students[0].score === 88, "the record: " + JSON.stringify(students[0]));\nassert(typeof students[0].score === "number", "score must be a number, not a string");\nassert(all("#list li")[0].textContent.trim() === "Ani: 88", "the line: " + JSON.stringify(all("#list li")[0].textContent.trim()));\nassert(sel("#name").value === "" && sel("#score").value === "", "both fields must be cleared");',
            },
            {
              name: { en: 'Bad input is refused', id: 'Bad input is refused' },
              check:
                'function submit(n, v) { sel("#name").value = n; sel("#score").value = v; sel("#form").dispatchEvent(new Event("submit", { cancelable: true, bubbles: true })); }\nvar before = students.length;\nsubmit("", "80");\nassert(students.length === before, "an empty name must not be added");\nassert(text("#error") === "Invalid input", "#error must say so, currently: " + JSON.stringify(text("#error")));\nsubmit("Budi", "abc");\nsubmit("Budi", "101");\nsubmit("Budi", "-1");\nassert(students.length === before, "invalid scores must not be added, currently: " + students.length);',
            },
            {
              name: { en: 'Zero and 100 are valid', id: 'Zero and 100 are valid' },
              check:
                'function submit(n, v) { sel("#name").value = n; sel("#score").value = v; sel("#form").dispatchEvent(new Event("submit", { cancelable: true, bubbles: true })); }\nvar before = students.length;\nsubmit("Zero", "0");\nsubmit("Hundred", "100");\nassert(students.length === before + 2, "0 and 100 must be accepted — check your range bounds");\nassert(text("#error") === "", "#error must be cleared after valid input, currently: " + JSON.stringify(text("#error")));',
            },
            {
              name: { en: 'The summary follows the data', id: 'The summary follows the data' },
              check:
                'var total = students.reduce(function (s, x) { return s + x.score; }, 0);\nvar avg = Math.round((total / students.length) * 10) / 10;\nvar want = students.length + " students, average " + avg;\nassert(text("#summary") === want, "#summary must be " + JSON.stringify(want) + ", currently: " + JSON.stringify(text("#summary")));',
            },
            {
              name: { en: 'save and load round-trip', id: 'save and load round-trip' },
              check:
                'var text_ = save();\nassert(typeof text_ === "string", "save() must return text, currently: " + typeof text_);\nvar copy = JSON.parse(text_);\nassert(copy.length === students.length, "the JSON must hold all records");\nassert(load(\'[{"name":"Zaki","score":70}]\') === true, "load() with valid JSON must return true");\nassert(students.length === 1 && students[0].name === "Zaki", "students must be replaced, currently: " + JSON.stringify(students));\nassert(text("#summary") === "1 students, average 70", "the summary must be updated too, currently: " + JSON.stringify(text("#summary")));',
            },
            {
              name: { en: 'Broken JSON changes nothing', id: 'Broken JSON changes nothing' },
              check:
                'var before = JSON.stringify(students);\nassert(load("not json") === false, "load() with broken text must return false");\nassert(JSON.stringify(students) === before, "students must not change when load fails");\nassert(error() === null, "do not let an error escape: " + error());',
            },
            {
              name: { en: 'The empty state reads properly', id: 'The empty state reads properly' },
              check:
                'assert(load("[]") === true, "load(\\"[]\\") must succeed");\nassert(text("#summary") === "No data yet", "#summary must be \\"No data yet\\", currently: " + JSON.stringify(text("#summary")));\nassert(all("#list li").length === 0, "the list must be empty");',
            },
          ],
          id: [
            {
            ...
            {
              name: { en: 'save and load round-trip', id: 'simpan dan muat pulang-pergi' },
              check:
        ...
        hints: [
          { en: 'One `render()` redraws the list and the summary from `students`. Call it after every change and the two can never disagree.', id: 'Satu `render()` menggambar ulang daftar dan ringkasan dari `kelas`. Panggil setelah tiap perubahan, maka keduanya tak mungkin berselisih.' },
          { en: 'Empty the list with `list.textContent = ""` before redrawing, or the items pile up.', id: 'Kosongkan daftarnya dengan `daftar.textContent = ""` sebelum menggambar ulang, atau itemnya menumpuk.' },
          ...
          { en: 'load parses inside try/catch, and only replaces `students` once the parse succeeded.', id: 'muat mengurai di dalam try/catch, dan baru mengganti `kelas` setelah penguraiannya berhasil.' },
          { en: 'Replace the contents rather than the binding: `students.length = 0` then push, since `students` is a const.', id: 'Ganti isinya, bukan ikatannya: `kelas.length = 0` lalu push, karena `kelas` adalah const.' },
        ],
        solution: {
          en: 'const students = [];\n\nconst form = document.querySelector("#form");\nconst nameEl = document.querySelector("#name");\nconst scoreEl = document.querySelector("#score");\nconst errorEl = document.querySelector("#error");\nconst summary = document.querySelector("#summary");\nconst list = document.querySelector("#list");\n\nfunction render() {\n  list.textContent = "";\n  for (const s of students) {\n    const li = document.createElement("li");\n    li.textContent = `${s.name}: ${s.score}`;\n    list.append(li);\n  }\n\n  if (students.length === 0) {\n    summary.textContent = "No data yet";\n    return;\n  }\n\n  const total = students.reduce((sum, s) => sum + s.score, 0);\n  const avg = Math.round((total / students.length) * 10) / 10;\n  summary.textContent = `${students.length} students, average ${avg}`;\n}\n\nform.addEventListener("submit", (event) => {\n  event.preventDefault();\n\n  const name = nameEl.value.trim();\n  const score = Number(scoreEl.value);\n\n  if (name === "" || scoreEl.value.trim() === "" || Number.isNaN(score) || score < 0 || score > 100) {\n    errorEl.textContent = "Invalid input";\n    return;\n  }\n\n  errorEl.textContent = "";\n  students.push({ name, score });\n  nameEl.value = "";\n  scoreEl.value = "";\n  render();\n});\n\nfunction save() {\n  return JSON.stringify(students);\n}\n\nfunction load(text) {\n  let data;\n  try {\n    data = JSON.parse(text);\n  } catch (err) {\n    return false;\n  }\n\n  if (!Array.isArray(data)) {\n    return false;\n  }\n\n  students.length = 0;\n  for (const s of data) {\n    students.push(s);\n  }\n  render();\n  return true;\n}\n\nrender();',
