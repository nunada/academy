    ...
    /* ---------------------------------------------- 2.1 building a worksheet */
    {
          ...
          steps: [
            {
              ...
              body: {
                en: 'One list of problem texts and one list of answers, appended together on every pass, stay lined up by position — `soal[0]` matches `kunci[0]`, and so on. It works, but nothing stops the two lists from silently drifting out of sync if either is edited alone later.',
            ...
            },
            {
              ...
              body: {
                en: 'A tuple `(soal, jawaban)` keeps a problem and its answer as one unit — appended together, they cannot be pulled apart by accident. Unpacking `for soal, jawaban in paket:` reads both halves of each pair at once.',
            ...
            },
            {
              ...
              id: 'q1',
              prompt: {
                en: 'paket holds 3 (soal, jawaban) tuples. What does `for soal, jawaban in paket:` do if paket is empty?',
              ...
              options: [
                ...
                { en: 'soal and jawaban become None', id: 'soal dan jawaban menjadi None' },
            ...
            },
            {
              ...
              lines: ['paket = [("4 + 1", 5), ("2 + 9", 11)]', 'for soal, jawaban in paket:', '    print(jawaban)'],
              explain: {
                en: 'soal is unpacked but simply unused here — Python does not require using every name a loop unpacks.',
            ...
            },
            {
              ...
              id: 'k1',
              prompt: {
                en: 'Read `n` and `s`. Seed with `s`. Build a list of `n` (soal, jawaban) pairs the same way as the concept above, then print only the `jawaban` values, one per line.',
              ...
              hints: [
                { en: 'Build the pairs first, then a second loop (or the same one) that only prints jawaban.', id: 'Bangun pasangannya dulu, lalu perulangan kedua (atau yang sama) yang hanya mencetak jawaban.' },
              ],
              solution:
                'import random\nn = int(input())\ns = int(input())\nrandom.seed(s)\npaket = []\nfor i in range(n):\n    a = random.randint(1, 10)\n    b = random.randint(1, 10)\n    paket.append((f"{a} + {b} = ?", a + b))\nfor soal, jawaban in paket:\n    print(jawaban)',
            },
      ...
      project: {
        ...
        requirements: [
          ...
          { en: 'Build a list of `n` pairs `(soal, jawaban)`, each drawing `a` and `b` from 1 to 10 the same way as module 2\'s lessons.', id: 'Bangun list `n` pasangan `(soal, jawaban)`, masing-masing mengambil `a` dan `b` dari 1 sampai 10 seperti pada pelajaran modul 2.' },
          { en: 'Print every `soal` first, one per line, then every `jawaban`, one per line.', id: 'Cetak semua `soal` dulu, satu per baris, lalu semua `jawaban`, satu per baris.' },
        ],
        starter: 'import random\nn = int(input())\ns = int(input())\nrandom.seed(s)\n',
        tests: [
          { name: { en: 'n=3, s=1', id: 'n=3, s=1' }, stdin: ['3', '1'], expectOutput: '3 + 10 = ?\n2 + 5 = ?\n2 + 8 = ?\n13\n7\n10' },
        ...
        hints: [
          { en: 'Two separate loops over the same paket list — one printing soal, one printing jawaban.', id: 'Dua perulangan terpisah atas list paket yang sama — satu mencetak soal, satu mencetak jawaban.' },
        ],
        solution:
          'import random\nn = int(input())\ns = int(input())\nrandom.seed(s)\npaket = []\nfor i in range(n):\n    a = random.randint(1, 10)\n    b = random.randint(1, 10)\n    paket.append((f"{a} + {b} = ?", a + b))\nfor soal, jawaban in paket:\n    print(soal)\nfor soal, jawaban in paket:\n    print(jawaban)',
        xp: 50,
    ...
    /* ------------------------------------------------- 2.2 automatic grading */
    {
          ...
          steps: [
            {
              ...
              body: {
                en: '`zip(kunci, jawaban)` pairs up the first items from each list, then the second, and so on — reading both at once instead of tracking an index by hand.',
            ...
            },
            {
              ...
              id: 'q1',
              prompt: {
                en: 'kunci and jawaban each have 3 items, but zip is accidentally given a kunci with only 2. What happens?',
            ...
            },
            {
              ...
              lines: ['benar = 4', 'total = 5', 'skor = benar / total * 100', 'print(round(skor, 1))'],
              explain: {
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              id: 'q1',
              prompt: {
                en: 'The score line uses len(kunci) rather than a fixed number like 3. Why?',
              ...
              options: [
                ...
                { en: 'len(kunci) runs faster than writing 3', id: 'len(kunci) berjalan lebih cepat daripada menulis 3' },
                { en: 'Because kunci might contain duplicates', id: 'Karena kunci mungkin berisi duplikat' },
            ...
            },
            {
              ...
              id: 'o1',
              prompt: {
                en: 'Assemble a loop printing Benar/Salah for two fixed lists.',
            ...
            },
            {
              ...
              id: 'k1',
              prompt: {
                en: 'Read `n`, then `n` key answers, then `n` student answers. Print `Benar` or `Salah` for each pair, then the score as a percentage rounded to 1 decimal place.',
      ...
      project: {
        ...
        requirements: [
          ...
          { en: 'Print `Benar` or `Salah` for each pair, in order.', id: 'Cetak `Benar` atau `Salah` untuk tiap pasangan, berurutan.' },
          { en: 'Print a final line: `Skor: X.X` — the percentage correct, rounded to 1 decimal place.', id: 'Cetak baris akhir: `Skor: X.X` — persentase yang benar, dibulatkan ke 1 angka desimal.' },
        ],
        starter: 'n = int(input())\nkunci = [int(input()) for _ in range(n)]\njawaban = [int(input()) for _ in range(n)]\n',
        tests: [
          { name: { en: '2 of 3', id: '2 dari 3' }, stdin: ['3', '5', '12', '7', '5', '10', '7'], expectOutput: 'Benar\nSalah\nBenar\nSkor: 66.7' },
        ...
        hints: [
          { en: 'Same as the lesson\'s own exercise, with the final line given a "Skor: " label via an f-string.', id: 'Sama seperti latihan di pelajarannya, dengan baris akhirnya diberi label "Skor: " lewat f-string.' },
        ],
        solution:
